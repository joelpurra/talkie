...
export default class ClipboardManager {
	...
	private async _getExistingTextareaOrNull(): Promise<Element | null> {
		const existingTextarea = document.querySelector(`#${this.copyPasteTargetElementId}`);

		return existingTextarea;
	}

	private async _getExistingTextarea(): Promise<HTMLTextAreaElement> {
		await this._assertIsInitialized();
	...
	private async _assertIsInitialized(): Promise<void> {
	...
	private async _ensureIsNotInitialized(): Promise<void> {
		const isInitialized = await this._isInitialized();
	...
	private async _injectBackgroundTextarea(): Promise<void> {
	...
	private async _initializeIfNecessary(): Promise<void> {
		const isInitialized = await this._isInitialized();
	...
	private async _removeBackgroundTextarea(): Promise<void> {
		const existingTextarea = await this._getExistingTextarea();

		existingTextarea.remove();
	}

	private async _getTextFromClipboard(): Promise<string | null> {
